...
### v1.3 → v1.4 (2026-09-03 Curator Resolution)

Brings this document in line with `claude/TAXONOMY_RESOLUTION_2026-09-03.md`, which resolved
five open taxonomy questions directly with the curator (Jaimy) and updated the live
registries (`tags.json`, `functions.json`) accordingly. This entry also folds in two
registry values (`architecture:nuraghe`, `assembly`) that the registries had already
carried since v0.2 but this document never listed — both are documented here for the first
time, not newly decided.

**New namespace:** `art` (§6, new §8a) — `art:rock-art`, `art:pictograph`,
`art:petroglyph`. Added because the existing `prehistoric:` and `archaeology:` namespaces
for rock/cave art both inherently claim a chronological or cave context that a period-neutral,
not-necessarily-cave physical-form tag cannot assume (a documented medieval carved relief is
rock art too). `prehistoric:rock-art`, `archaeology:cave-art`, and `cave:art` are unaffected
and remain in active use — `art:rock-art` is the new general term for the physical form,
not a replacement for any of them.

**Deprecated tag:** `military:broch` → `architecture:broch` (§7, §16) — a broch is a
building form, not a claim about military use; "uncertainty never becomes a military
claim" was the underlying rule. `architecture:nuraghe` (already in use for the parallel
Sardinian tower form) is documented in §7 for the first time here, alongside its new sibling.

**New Functions:** `defensive` (§21) — narrower than `military`, for a documented defensive
structure without implying garrisons, campaigns, or wider military claims; `irrigation`
(§21) — distinct from both `agricultural` and `water-supply`, neither of which captures
irrigation specifically without risking false positives on unrelated records. `assembly`
(§21) is also documented here for the first time — it already existed in the registry as of
v0.2, covering "a formal communal, legal, political or deliberative assembly place," and was
reused rather than adding a near-duplicate `civic-assembly`.

**Field retired:** `secondaryType` (§37) — retired after a reviewed migration of the one
live record that used it. Its value is preserved in that record's migration provenance, not
dropped; the site's religious use is now represented as `functions: ["religious"]` and its
named sub-feature as a `components` entry, and its `canonicalType` was left untouched
(a separate, still-open review question).

**No Type changes, no new namespace beyond** **`art`****, and no change to any other tag or
function vocabulary.** See the resolution document for the full record-by-record basis of
every change above.
...
Approved namespaces. Every v1.1/v1.2/v1.3 tag addition fit within the original fourteen;
`art` (v1.4, §8a) is the first genuinely new namespace this taxonomy has needed, added only
after confirming none of the existing fourteen could hold a period-neutral physical-form tag
for rock/cave art without a false chronological or cave-context claim:
...
`architecture:nuraghe` and `architecture:broch` (§16, v1.4) are the current tags for these
regional tower/fortification forms — a nuraghe (Sardinian) or broch (Scottish/Atlantic) is a
building form, and neither is itself a claim of military use. `military:broch` (§16) is
deprecated in favor of `architecture:broch` as of the 2026-09-03 resolution; where a record's
own text documents a genuinely defensive use, add `function: defensive` (§21) separately —
never `function: military`, which claims more than the tag or the evidence supports.
...
`art:rock-art` is the general, period-neutral physical-form tag for rock/cave art of any
date — it applies equally to a Palaeolithic cave painting and a documented medieval carved
relief, and is not restricted to prehistoric sites the way `prehistoric:rock-art` (§18) or
`archaeology:cave-art` (§8) inherently are. Use `art:pictograph` or `art:petroglyph` as
technique refinements only where the record's own source text actually distinguishes
painted from carved/incised work; leave the record with `art:rock-art` alone, unrefined,
rather than guessing a technique the text doesn't specify.

`art:rock-art` does not replace `prehistoric:rock-art`, `archaeology:cave-art`, or
`cave:art` (§9) — all three remain in active use for the cases they already correctly
describe (a prehistoric-dated rock art site; the archaeological-condition framing; "this
cave has art," undifferentiated). Adding `art:rock-art` to a record does not mean removing
any of the others if they already apply and remain accurate.

A cave context is separately covered by the existing `cave:*` namespace (§9) — a record
already typed `Cave` (§4.8) already carries that context via its Type, so `art:rock-art`
does not by itself imply anything about whether the art is inside a cave; open-air rock art
(§4.3) can carry it just as validly as cave-located rock art (§4.8).
...
**`military:broch`** **is deprecated (v1.4)** in favor of `architecture:broch` (§7) — a broch is
a building form, not itself a military claim. Existing records were migrated; do not apply
`military:broch` to new records. `military:nuraghe` is unaffected by this decision — see §7
for `architecture:nuraghe`, which has been in parallel use since before this resolution.
...
**`defensive`** (v1.4) is deliberately narrower than `military`: it records that a
structure's documented purpose was defense, without the wider claims `military` carries
(garrisons, campaigns, standing forces). Use `defensive` where the record's own text
supports a defensive purpose but not those broader claims — a nuraghe or broch described as
a "defensive tower" gets `defensive`, not `military`, unless the text itself documents
something more.

**`irrigation`** (v1.4) is for a documented irrigation function, kept distinct from both
`agricultural` (too general — covers non-irrigation farming activity too) and
`water-supply` (covers non-agricultural water systems, e.g. urban supply) so that a record
with only one of those two unrelated functions is never miscounted as also having
irrigation.

**`assembly`** (already in the registry since v0.2, documented here for the first time) is
for "a formal communal, legal, political or deliberative assembly place" — e.g. a Viking-age
þing/assembly site. Prefer this existing value over inventing a near-duplicate like
`civic-assembly`, which would only split future queries between the two (Rule 7).
...
**`secondaryType`** **was retired (v1.4, 2026-09-03 resolution)** after a reviewed migration of
the one live record that carried it — its value was preserved in that record's migration
provenance, not dropped, and its actual religious use is now represented properly as
`functions: ["religious"]` plus a `components` entry for the named sub-feature it pointed
at. Do not repopulate `secondaryType` on any record going forward; represent a secondary
identity through Functions or `components` instead, per that record's own precedent.
