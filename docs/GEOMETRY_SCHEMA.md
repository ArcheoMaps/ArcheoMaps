...
```json
{
  ...
  "name": "Hadrian's Wall — main alignment",
...
- `name`: optional feature-level display-name override used only when this Geometry feature needs a label distinct from its linked Entity's `preferredName`; otherwise consumers resolve the name through `entityId`.
