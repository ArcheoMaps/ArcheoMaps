...
Version: 1.7 — NATURAL HERITAGE AND LINKED GEOMETRY WORKING SPECIFICATION
...
### v1.7 Phase 1 Geometry Layer System amendment (curator-approved, 2026-09-14)

This additive amendment formalizes the first Geometry Layer System contract without changing the v1.7 Type, Tag, Function, Culture, or natural-heritage vocabularies and without creating Taxonomy v1.8.

- **Phase political-control compatibility (§26):** the flat `politicalEntities[]` arrays at Site and Phase level remain preserved compatibility projections. New sourced and dated control is canonical in `politicalEntityAssertions[]`; phase-specific assertions reference a stable Site-scoped `phaseId`. No existing flat value is automatically migrated.
- **Geometry Relationships (§63):** `geometry` is added as an internal `targetKind`. The Relationship Type vocabulary gains `located-on`, `crosses`, `adjacent-to`, and `represents-section-of`; existing `part-of` and `associated-with` remain reusable. Geometry links use the existing §63 evidence and review model.
- **Separate governed resources:** `docs/GEOMETRY_SCHEMA.md`, `docs/GEOMETRY_DATASET_REGISTRY.md`, and `docs/ENTITY_REGISTRY.md` define spatial claims, dataset provenance, and entity identity respectively. These are linked rather than embedded repeatedly in Site records.
- **Housekeeping:** §6 now states the correct total of 21 approved Tag namespaces. §59's registry lifecycle is single-sourced as `active`, `proposed`, `deprecated`, `rejected`, and `quarantined`; `pending` is not a valid registry status and normalizes to `proposed` where encountered.

### v1.6 → v1.7 (Natural Heritage and Linked Geometry, 2026-09-11)

Version 1.7 expands ArcheoMaps from an exclusively historical/archaeological place taxonomy into a taxonomy that can also represent UNESCO Natural and Mixed properties without forcing purely natural records into `Other`.

- **New canonical Type:** `Natural Site` (§4.13). The controlled Type vocabulary now contains 15 values. `Natural Site` is used when a record fundamentally represents a naturally formed landscape, landform, geological feature, ecosystem, habitat, waterscape, or palaeontological locality.
- **New natural-science Tag namespaces:** `biome`, `ecology`, `geology`, `hydrology`, and `landform`, plus a natural-site use of the existing `marine` namespace (§14a–§14f). `landscape:natural` is added as the neutral generic natural-property Tag.
- **UNESCO natural criteria:** criteria `(vii)`–`(x)` are stored as Heritage-designation metadata, not inferred from marker artwork and not duplicated as Tags (§36, §57).
- **Twenty-six natural marker identities:** N01–N26 are documented as presentation/selection identities derived from recurring UNESCO natural-property forms. They do not create 26 Types and do not replace source-supported per-record classification (§36.1).
- **Administrative status remains separate:** “reserve”, “sanctuary”, “protected”, and similar legal or programme statuses belong in Heritage/Protection assertions, not physical-form Tags (§36.1, §57).
- **Linked optional geometry:** Site records may reference separate GeoJSON geometry resources for official property boundaries, buffer zones, natural-feature extents, approximate extents, and linear features. These overlays are off by default, may be revealed temporarily by selecting the linked marker, and may be pinned independently through Layers (§54).
- **Corrected former exclusion:** the v1.6 rule that sent purely natural features to `Other` or review is superseded. A sufficiently evidenced natural record now uses `canonicalType = Natural Site`; `Other` and `null + workflow` retain their distinct existing meanings (§4.15, §38).
- **No catalogue or runtime migration:** this specification update does not itself alter `archeomaps_data.json`, `index.html`, Scribe, sprite atlases, or existing records.
...
**No Type changes in v1.6.** The 14-Type vocabulary remained unchanged until v1.7 added `Natural Site` for the newly admitted natural-heritage scope.
...
**Clarified guidance (no new vocabulary):** §4.3 Monument — added open-air rock art as an example. §4.9 Archaeological Site — added "ruins of unclear type" and "isolated civic architecture" as explicit fallback examples. The former Landscape/Other sections (now §4.14/§4.15) added guidance distinguishing natural landmarks with documented cultural significance from those with none; v1.7 later supersedes their former exclusion of purely natural records.
...
`Observatory` (§4.12) and `Natural Site` (§4.13) are the only Types added since v1.0. Each was added only after determining that the existing vocabulary could not represent the fundamental identity without distortion. Every narrower natural distinction remains a Tag or another dimension, not a Type.
...
Use when a record fundamentally represents a naturally formed landscape, landform, geological feature, ecosystem, habitat, waterscape, or palaeontological locality rather than a human-created historical or archaeological place.

Examples include forests, deserts, mountain ranges, cliffs, canyons, volcanoes, geothermal systems, glaciers, fossil localities, rivers, lakes, wetlands, waterfalls, mangroves, islands, coral reefs, and other terrestrial or marine ecosystems.

Possible Tags are drawn from the controlled natural-science namespaces in §14a–§14f. Typical examples include:
...
hydrology:river
marine:coral-reef
ecology:migratory-habitat
```

Rules:

1. `Natural Site` describes fundamental identity; UNESCO designation does not itself determine Type.
2. A Natural or Mixed UNESCO property may use `Natural Site` only when that is what the ArcheoMaps record itself fundamentally represents. Mixed status never automatically chooses the Type.
3. A natural cave whose cave identity is the record's central subject may remain `Cave`; a culturally significant mountain landscape may remain `Landscape`. Apply the normal record-scope rule rather than classifying from one component.
4. Legal protection, reserve, park, sanctuary, and World Heritage status belong in Heritage/Protection assertions (§57), not physical-form Tags.
5. The representative point marker and the property's spatial extent are separate. Link authoritative or derived geometry through `geometryRefs` (§54); do not place large polygon coordinate arrays inside the taxonomy fields.
6. The N01–N26 marker identities (§36.1) are presentation choices. A sprite never supplies classification evidence and does not restrict a record to one Tag.
...
A natural landmark whose primary documented significance is cultural, mythological, religious, or archaeological belongs here — e.g. `landscape:sacred` — even if it has no constructed remains. A record whose primary identity is instead the natural feature or ecosystem uses `Natural Site` (§4.13). Where both cultural and natural values occur, classify the scope of the specific ArcheoMaps record and preserve the other value through Tags, Components, Relationships, and Heritage metadata as supported.
...
- **Confidently "none of the 14 specific Types fit"** → `canonicalType: "Other"`. This is a terminal, positive classification. It does not, by itself, enter a workflow review queue — the classification decision has already been made and is not pending.
...
Purely natural identity is no longer a reason to use `Other`: sufficiently evidenced natural features use `Natural Site` (§4.13). `Other` remains available only when the evidence positively establishes that none of the 14 specific Types fits.
...
There are **21 approved Tag namespaces**. Version 1.7 adds the natural-science namespaces `biome`, `ecology`, `geology`, `hydrology`, and `landform`; `marine` remains distinct from the historical/archaeological `maritime` namespace:
...
Landscape tags describe an interpretive landscape identity or setting. Natural physical form uses §14a–§14f. The following v1.6 values remain readable as deprecated compatibility inputs; new records use the more precise replacement:

```
landscape:mountain -> landform:mountain
landscape:desert   -> biome:desert
landscape:river    -> hydrology:river
landscape:island   -> landform:island
landscape:coastal  -> marine:coastal
landscape:forest   -> biome:forest
```

Do not rewrite an existing value without preserving its source and migration provenance. See §4.13 for Natural Sites and §4.14 for culturally or archaeologically significant Landscapes.
...
Assign only source-supported biomes. Broad marker identity N08 does not justify adding grassland, steppe, and savanna simultaneously; select the value or values actually documented for the property.
...
Use individual precise values rather than combined registry strings such as `mountain-or-highland` or `canyon-or-gorge`. Combined wording is acceptable for a sprite identity, not for canonical data.
...
`geology:fossil-site` identifies a palaeontological locality. Individual fossil or hominin evidence belongs in `evidence[]` (§48), not in Tags.
...
Hydrological Tags describe the feature represented by the record. Detailed river geometry may be a separate linked `linear-feature` resource (§54).
...
The `marine` namespace describes natural marine environment. The existing `maritime` namespace (§15) describes human maritime activity, infrastructure, or association; the two are not synonyms.

## 14f. Ecology Tags

```
ecology:wildlife-habitat
ecology:bird-habitat
ecology:migratory-habitat
ecology:biodiversity
ecology:ecosystem
```

Ecology Tags require explicit ecological evidence. “Reserve” and “sanctuary” are protection/designation concepts and never populate these Tags by name alone (§57).
...
**Political entities — canonical assertion model (curator-approved Geometry Layer System v0.1 amendment, 2026-09-14):** the flat `politicalEntities[]` array on a Phase is a legacy/compatibility field, equivalent in status to the Site-level flat array. Both remain preserved as-is and are never automatically migrated because their strings lack the chronology, control type, assignment method, confidence, and evidence needed to construct richer assertions safely. New sourced and dated political control is canonical in the Site record's `politicalEntityAssertions[]` array as defined by `docs/ENTITY_REGISTRY.md`. A phase-specific assertion must reference the applicable Site-scoped Phase instance through `phaseId`; it must never use a bare Historical Phase Type. New phase-specific control must not be recorded solely by appending a string to the Phase's flat `politicalEntities[]` array.
...
Natural World Heritage criteria are stored verbatim as Heritage metadata:

```
vii   superlative natural phenomena or exceptional natural beauty
viii  Earth history, the record of life, geological processes, or landforms
ix    ecological and biological processes
x     biodiversity and significant habitats
```

Criteria never auto-generate canonical Type or Tags. The source description must support each classification. A `mixed` property is not automatically `Natural Site`; apply the normal record-scope rule.

### 36.1 Natural World Heritage marker identities N01–N26

These are ArcheoMaps presentation identities derived from recurring natural World Heritage forms. They are not an official UNESCO typology and do not expand the canonical Type list beyond `Natural Site`. Each identity selects a standard marker only after the record's Type and Tags have been established from evidence.

| ID | Marker identity | Usual canonical Type | Source-supported classification Tags |
|---|---|---|---|
| N01 | Generic natural or protected landscape | `Natural Site` | `landscape:natural` |
| N02 | Temperate forest | `Natural Site` | `biome:temperate-forest` |
| N03 | Tropical rainforest | `Natural Site` | `biome:tropical-rainforest` |
| N04 | Mountain or highland | `Natural Site` | `landform:mountain`, `landform:mountain-range`, and/or `landform:highland` |
| N05 | Cliff or escarpment | `Natural Site` | `landform:cliff` and/or `landform:escarpment` |
| N06 | Canyon or gorge | `Natural Site` | `landform:canyon` and/or `landform:gorge` |
| N07 | Desert or dune landscape | `Natural Site` | `biome:desert`; optionally `landform:dune` |
| N08 | Grassland, steppe or savanna | `Natural Site` | one or more of `biome:grassland`, `biome:steppe`, `biome:savanna` |
| N09 | Tundra or polar landscape | `Natural Site` | `biome:tundra` and/or `biome:polar` |
| N10 | Volcano | `Natural Site` | `landform:volcano` |
| N11 | Geothermal site | `Natural Site` | `geology:geothermal` |
| N12 | Glacier or icefield | `Natural Site` | `landform:glacier` and/or `landform:icefield` |
| N13 | Geological formation | `Natural Site` | `geology:geological-formation` |
| N14 | Fossil or palaeontological site | `Natural Site` | `geology:fossil-site`; fossil remains belong in `evidence[]` where modelled |
| N15 | River landscape | `Natural Site` | `hydrology:river` |
| N16 | Lake landscape | `Natural Site` | `hydrology:lake` |
| N17 | Wetland or marsh | `Natural Site` | `biome:wetland` and optionally `biome:marsh` |
| N18 | Waterfall | `Natural Site` | `landform:waterfall` |
| N19 | Mangrove landscape | `Natural Site` | `biome:mangrove` |
| N20 | Coastal or marine landscape | `Natural Site` | `marine:coastal` and/or `marine:open-water` |
| N21 | Island or archipelago | `Natural Site` | `landform:island` and/or `landform:archipelago` |
| N22 | Coral reef or atoll | `Natural Site` | `marine:coral-reef` and optionally `landform:atoll` |
| N23 | Marine reserve | `Natural Site` | actual marine form, such as `marine:open-water`, `marine:coastal`, or `marine:coral-reef`; reserve status remains separate |
| N24 | Wildlife reserve or habitat | `Natural Site` | `ecology:wildlife-habitat`; reserve status remains separate |
| N25 | Bird sanctuary or migratory habitat | `Natural Site` | `ecology:bird-habitat` and/or `ecology:migratory-habitat`; sanctuary status remains separate |
| N26 | Biodiversity or ecosystem site | `Natural Site` | `ecology:biodiversity` and/or `ecology:ecosystem` |

The broad fallback identities N01 and N23–N26 may be selected when they are the most faithful supported presentation, but they must not erase more precise Tags. Conversely, marker artwork never supplies missing Tags.
...
13. If a site's classification is genuinely uncertain, use a **null value plus a workflow review flag** — never fabricate certainty. `Other` is reserved for the different, confident case where the evidence positively supports "none of the 14 specific Types fit"; it is a terminal classification, not a review-pending state. See §4.15 (v1.5.1 correction, T15-P0-004) for the full distinction and why the two must not be treated as interchangeable.
...
15. A record whose fundamental identity is a sufficiently evidenced purely natural feature, ecosystem, habitat, waterscape, geological feature, or palaeontological locality uses `canonicalType = Natural Site`; it is not forced into `Landscape` and is no longer sent to `Other` merely for lacking cultural/historical content. Genuine uncertainty still uses `canonicalType: null` plus workflow review. Natural caves may remain `Cave` when cave identity is the record's primary scope (§4.13).
...
If a source does not provide enough evidence to classify a site at all: leave `canonicalType: null` and attach a workflow review flag. Do NOT guess, and do not use `Other` for this case — `Other` (§4.15) is reserved for the confident, positive conclusion that none of the 14 specific Types fit; it does not enter review. See §4.15 for the full v1.5.1 correction (T15-P0-004).
...
Large extents and optional overlays SHOULD be stored as separate GeoJSON resources and linked from the Site rather than embedded repeatedly in catalogue records:

```json
{
  "geometryRefs": [
    {
      "id": "geometry-unesco-123-boundary",
      "role": "property-boundary",
      "preferred": true,
...
The referenced GeoJSON Feature carries the stable geometry ID and Site relationship:
...
```json
{
  ...
  "properties": {
    "siteId": "unesco-123",
    "geometryRole": "property-boundary",
...
Initial linked Geometry Role vocabulary:
...
`property-boundary` is the official extent of the designated property; it must not be presented as the boundary of an entire desert, forest, mountain range, or other natural system unless the source explicitly establishes that equivalence. `buffer-zone` remains visibly and semantically distinct. `natural-feature` describes the feature itself. `linear-feature` supports walls, roads, routes, rivers, and comparable linework. `approximate-extent` is explicitly non-authoritative and must carry its derivation and confidence.

Linked geometry display behavior:

1. Optional geometry layers are globally off by default.
2. Selecting a linked Site marker may reveal its geometry temporarily without changing the saved Layers state.
3. Deselecting the Site hides a temporarily revealed geometry unless the user pinned or enabled it in Layers.
4. A geometry enabled in Layers remains visible independently of marker selection.
5. Polygon, MultiPolygon, LineString, and MultiLineString resources use the same interaction contract; the Great Wall and a UNESCO forest boundary differ in geometry family, not selection behavior.
6. Geometry loading should be lazy or spatially partitioned so default map startup does not fetch every optional extent.
7. Every geometry resource carries provenance, confidence/authority, and an explicit role. Derived or approximate outlines are never displayed as official boundaries.
...
5. Routes, borders, coastlines, ice sheets, rivers, natural-property extents, and other regional or time-varying geometries remain separate linked Layer resources even though they use the same geometry standards.
...
Legal or administrative protection may be represented independently:
  ...
  "protections": [
    {
      ...
      "status": "active",
      "designatedAt": 1980,
      "sourceIds": ["source-protection-001"]
    }
  ]
}
```

Initial broad protection vocabulary:
...
These values record administrative status only. They do not generate `Natural Site`, ecology Tags, UNESCO criteria, or a marker assignment without independent evidence.
...
This five-value list is exhaustive. `pending` is not a valid registry status; any legacy or imported registry entry carrying `pending` must normalize to `proposed` while preserving the original raw value in provenance. Ordinary prose may still use the word “pending” descriptively, but implementations must not persist it as a controlled `status` value.
...
> "How is this place connected to other ArcheoMaps records, governed Geometry resources, or external resources?"
...
**External targets:** where `targetKind: "external"` identifies a Wikidata, GeoNames, or other outside-authority entity, there is no ArcheoMaps-side record to hold a reciprocal assertion. `reciprocal` must be `false`, `targetId` is replaced by a `targetUri` (or similar external reference) instead of an internal stable ID, and no pair-matching validation applies — the assertion stands alone, sourced and reviewed like any other, but is structurally incapable of having a reciprocal half.

**Geometry targets (curator-approved Geometry Layer System v0.1 amendment, 2026-09-14):** `targetKind: "geometry"` uses `targetId` to reference an internal Geometry feature governed by `docs/GEOMETRY_SCHEMA.md`. It reuses the existing `pairId`, `chronology`, `confidence`, `sourceIds`, `review`, reciprocal, and notes fields unchanged. For v0.1, Geometry relationships must set `reciprocal: false`: Geometry features are ArcheoMaps-governed but do not yet define a `relationships[]` array capable of storing the reciprocal half. If Geometry resources later gain relationships, this constraint must be revisited and §63.2's currently fixed, non-extensible inverse-type table must be explicitly extended to cover `located-on`, `crosses`, `adjacent-to`, and `represents-section-of` before reciprocal Geometry links are permitted.

`geometryRefs[]` retains its existing §54 boundary and overlay-display role, with its existing `{id, role, preferred, defaultVisible, revealOnSelection, sourceIds}` shape. Semantic Site-to-Geometry assertions belong in `relationships[]`; `geometryRefs[]` must not acquire a competing `relationship` field.
...
8. `located-on`, `crosses`, `adjacent-to`, and `represents-section-of` are the initial Geometry relationship types. Existing `part-of` and `associated-with` must be reused where they already express the intended link; underscored or otherwise duplicated aliases must not be introduced.
