...
function processPackage(collection, dataset) {
  ...
  features.forEach((feature, index) => {
    ...
    const id = feature?.properties?.id;
    if (id && idCounts.get(id) > 1) issues.push(issue('DUPLICATE_ID', `Duplicate geometry id: ${id}`, `/features/${index}/properties/id`));
    ...
    if (hardIssues.length) quarantined.push({ index, id: id || null, issues, feature });
      ...
      accepted.push({ ...feature, properties: canonicalize(feature.properties) });
      const featureNotices = issues.filter(item => item.code === 'ROLE_OVERRIDE');
      if (featureNotices.length) notices.push({ index, id: id || null, issues: featureNotices });
    }
  });
  if (Number.isInteger(dataset.featureCount) && dataset.featureCount >= 0 && dataset.featureCount !== features.length) fatal.push(issue('FEATURE_COUNT_MISMATCH', `Dataset declares ${dataset.featureCount}; input contains ${features.length}.`, '/featureCount'));
  const observed = [...new Set(features.map(f => f?.geometry?.type).filter(Boolean))].sort();
  if (Array.isArray(dataset.geometryTypes) && stableJson(observed) !== stableJson([...dataset.geometryTypes].sort())) fatal.push(issue('GEOMETRY_TYPES_MISMATCH', `Dataset geometryTypes ${JSON.stringify(dataset.geometryTypes)} do not match observed ${JSON.stringify(observed)}.`, '/geometryTypes'));
  ...
  notices.sort((a, b) => (a.id || '').localeCompare(b.id || '') || a.index - b.index);
  return { fatal, accepted, quarantined, notices };
}
...
function run(args) {
  ...
  const report = {
    ...
    counts: { input: Array.isArray(collection.features) ? collection.features.length : 0, accepted: result.accepted.length, quarantined: result.quarantined.length, notices: result.notices.length, fatal: result.fatal.length },
